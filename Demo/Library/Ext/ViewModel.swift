    ...
    func notify<Element: Object>(change: RealmCollectionChange<Results<Element>>) {
        ...
        case .update(_, let deletions, let insertions, let modifications):
            if !deletions.isEmpty {
                ...
                self.delegate??.viewModel?(self, didChangeItemsAt: indexPaths, changeType: .delete)
            }
            if !insertions.isEmpty {
                let indexPaths = insertions.map { IndexPath(item: $0, section: 0) }
                self.delegate??.viewModel?(self, didChangeItemsAt: indexPaths, changeType: .insert)
            }
            if !modifications.isEmpty {
                let indexPaths = modifications.map { IndexPath(item: $0, section: 0) }
                self.delegate??.viewModel?(self, didChangeItemsAt: indexPaths, changeType: .update)
            }
        default:
            break
        }
